...
const {projectPathConfig: {path: projectRootDir, pagesDir, defaultPublicDir}} = require('../util/configManager'); // TemporaryDir,
...
function renderSub(response, subPath) {
    ...
    response.render('sub', {entryScript, isDev: true});
}
...
function getRenderInfo(url) {
    ...
    const expectPagePath = path.resolve(projectRootDir, pagesDir, subPath, 'index.vue');
