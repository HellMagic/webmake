...
const {projectPathConfig: {configDir: {apiProxyConfigFile}}} = require('./configManager');
...
function injectHostHeader(apiProxyConfig) {
    Object.values(apiProxyConfig).forEach(item => {
        if (Boolean(item.target) && typeof item.target === 'string') {
            const matchResult = item.target.match(/^https?:\/\/([\w.-]+)\/?/);
            if (!matchResult || !matchResult[1]) throw new Error(`从 apiProxyConfig 的 target 解析 Host 失败：target: ${item.target}`);
            item.headers = {
                'Host': matchResult[1]
            }
        }
