...
// 对外暴露的 向 Service 中注册各种回调函数（大部分都是修改 webpack 配置的）的接口函数。
...
class PluginAPI {
    ...
    assertVersion(range) {
        if (typeof range === 'number') {
            if (!Number.isInteger(range)) {
                throw new TypeError('Expected string or integer value.');
            }
   ...
   */
    registerCommand(name, options, fn) {
        if (typeof options === 'function') {
            fn = options;
            options = null;
        }

        this.service.commands[name] = {fn, opts: options || {}};
    }
   ...
   */
    genCacheConfig(id, partialIdentifier, configFiles = []) {
        ...
        const variables = {
            ...
            test: Boolean(process.env.VUE_CLI_TEST),
            config: [
